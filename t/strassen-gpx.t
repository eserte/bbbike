...
EOF
		...
		sub {
		    ...
		    {
			my @nodes = $doc->findnodes('//*[local-name(.)="metadata"]');
...
EOF
		...
		sub {
		    my $doc = shift;
		    {
			my @nodes = $doc->findnodes('//*[local-name(.)="metadata"]/*[local-name(.)="name"]');
		    ...
		    {
			my @nodes = $doc->findnodes('//*[local-name(.)="metadata"]/*[local-name(.)="link"]');
	...
	for my $utf8 (0, 1) {
	    my $route_data = <<EOF;
#: title: Global directive title with äöü
#:
Start	X 100,100
Goal	X 300,300
EOF
		...
		xpath_checks $xml_res, 2,
			...
			my @nodes = $doc->findnodes('//*[local-name(.)="rte"]/*[local-name(.)="name"]');
			is scalar(@nodes), 1, 'exactly one rte/name node';
			is $nodes[0]->textContent, 'Global directive title with äöü', 'route name from global directive in bbd file';
		    };
	    }
	    {
