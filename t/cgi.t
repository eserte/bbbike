...
#
# $Id: cgi.t,v 1.47 2007/08/12 20:17:42 eserte Exp $
# Author: Slaven Rezic
#
...
for my $cgiurl (@urls) {
    ...
    $res = $ua->request($req);
    ok($res->is_success, "Dudenstr. -> Grimmstr.") or diag $res->as_string;
    $content = uncompr($res);
...
# 	    }
	...
	$req = new HTTP::Request
	    (GET => $image_url, $hdrs);
	$res = $ua->request($req);
	ok($res->is_success, "Getting $image_url") or diag $res->as_string;
	...
	if ($content !~ qr|SRC="(.*/mapserver/brb/tmp/brb.*.png)"|) {
		...
		(GET => $image_url, $hdrs);
	    $res = $ua->request($req);
	    ok($res->is_success, "$image_url was OK") or diag $res->as_string;
	    is($res->content_type, 'image/png', "$image_url is a PNG");
	    ok(length($content) > 0, "Image is non-empty");
	}
    }
 ...
 SKIP: {
	...
	$res = $ua->request($req);
	ok($res->is_success, "Dudenstr. -> Guben") or diag $res->as_string;
	$content = uncompr($res);
	...
	$res = $ua->request($req);
	ok($res->is_success, "Map plot request") or diag $res->as_string;
	...
	$res = $ua->request($req);
	ok($res->is_success, "Route request") or diag $res->as_string;
	$content = uncompr($res);
	my $route = $cpt->reval($content);
