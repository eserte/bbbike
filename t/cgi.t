...
for my $cgiurl (@urls) {
    ...
    $content = uncompr($res);
    ok($content, qr/Dieses Programm sucht .*Fahrrad-.*Routen in Berlin/);
    ...
    ok($res->is_success, 1, $res->as_string);
    $content = uncompr($res);
    ok($content, qr/Start.*Dudenstr\./);
    ok($content, qr/Grimmstr\..*Kreuzberg.*10967/);
    ...
    ok($res->is_success, 1, $res->as_string);
    $content = uncompr($res);
    ok($content, qr/Mehringdamm.*Platz.*Tempelhofer/);
    ok($content, qr/Urban.*Fichte/);
	...
	ok($content, qr/Gubener Str.!Friedrichshain/, "Can't find Gubener Str. in " . $content);
	ok($content, qr/Guben!\#ort!/, "Can't find Guben in " . $content);
    }
    ...
    ok($res->is_success, 1, $res->as_string);
    $content = uncompr($res);
    ok($content, qr/Anfangsbuchstabe.*D/);
    ok($content, qr/Dudenstr/);
	...
	ok($res->is_success, 1, $res->as_string);
	my $content = uncompr($res);
	ok($content, qr/Darzustellende Ebenen/);
