...
sub _bbd2gpx_libxml {
    ...
    my $auto_metadata = delete $args{-autometadata};
    ...
    if ($auto_metadata) {
	my $metadata = $gpx->addNewChild(undef, 'metadata');
	if ($meta && defined $meta->{name}) {
	    $metadata->appendTextChild('name', $meta->{name});
	}
	my $link = $metadata->addNewChild(undef, 'link');
	$link->setAttribute('href', 'bbbike.de');
	$link->appendTextChild('text', 'BBBike');
	## <time> is generated by some applications, but is not valid gpx 1.1
	#require POSIX;
	#$metadata->appendTextChild('time', POSIX::strftime('%Y-%m-%dT%H:%M:%SZ', gmtime));
    }
...
sub _bbd2gpx_twig {
    ...
    if ($auto_metadata) {
	my $metadataxml = XML::Twig::Elt->new('metadata');
	$metadataxml->paste(last_child => $gpx);
	if ($meta && defined $meta->{name}) {
	    XML::Twig::Elt->new('name', {}, $meta->{name})->paste(last_child => $metadataxml);
	}
	my $linkxml = XML::Twig::Elt->new('link', {href => 'bbbike.de'});
	$linkxml->paste(last_child => $metadataxml);
	## <time> is generated by some applications, but is not valid gpx 1.1
	#require POSIX;
	#XML::Twig::Elt->new('time', {}, POSIX::strftime('%Y-%m-%dT%H:%M:%SZ', gmtime))->paste(last_child => $linkxml);
    }
