...
#
# $Id: StrassenNetzHeavy.pm,v 1.21 2005/04/30 07:46:01 eserte Exp $
#
...
sub load_user_deletions {
	...
	die "Coordinate count should be 2, but is @{[ scalar @{ $ret->[Strassen::COORDS()] } ]}. The output file should be generated by the 'user deletions' feature of bbbike. Position " . $s->pos
	    if @{ $ret->[Strassen::COORDS()] } != 2;
	$net->add_to_deleted($ret->[Strassen::COORDS()]->[0],
			     $ret->[Strassen::COORDS()]->[1],
			     $on_callback,
			     $del_token);
	$set{$ret->[Strassen::COORDS()]->[0]}->{$ret->[Strassen::COORDS()]->[1]}++;
    }
    if (!$do_merge) {
