...
daß die ganzen remote-Kommandos verwendet. Hier eine alte Liste:
...
Radrouten auch als Routenliste bereitstellen. Das könnte man hier
L<http://bbbike.radzeit.de/mapserver/brb/radroute.html> als zweiten
Button bereitstellen. Die Verlinkung würde dann auf bbbike.cgi mit
einer Variation des localroutefilelist-Parameters gehen. Detail ist
noch unklar: entweder bereite ich pro Radroute eine .bbr-Datei vor
(wie kann man diese am besten automatisch erstellen?) oder ich bringe
bbbike.cgi bei, auch mit einem POST von Koordinaten umzugehen. Dann
müsste ich aber Javascript ins Spiel bringen, weil das submit entweder
auf mapserver oder auf bbbike gehen müsste (ein paar hidden-Variablen
umsetzen, und sicherheitshalber den zweiten Submit-Button in <script>
einschließen, damit nicht javascript-fähige Browser nur einen Button
sehen). Oder ich doppele alle Formulare, aber das wäre Overkill (sind
jetzt schon >100kB).
...
Wie behandelt man Abschnitte, in denen die Hin- und Rückstrecke nicht
gleich ist (Beispiel: E<quot>CS;E<quot>, z.B. Großer Stern)?

Wie behandelt man Alternativen?

Soll man auch den Rückweg bzw. die andere Richtung bereitstellen? Wie?
