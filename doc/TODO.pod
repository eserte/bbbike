...
-> Die Funktionalität ist implementiert, ohne Code zu teilen
   (wahrscheinlich zu schwierig). Was fehlt: Tests, ob tatsächlich die
   gleichen Routen dabei herauskommen.
...
-> Ist Touratech im Zeitalter von kml und gpx noch relevant? Kurz
   recerchieren und ggfs. dieses Item löschen.
...
(Beispiel?). Bei Situationen wie Falkensteinstr./Oberbaumstr. sollte
trotz Geradeausfahrt die Nebenstraße gesondert gekennzeichnet werden
(z.B. E<quot>nicht der Hauptstraße folgenE<quot>). Bei einfachen
Kurvensituationen ohne Kreuzung (Beispiel?) braucht keine
Richtungsangabe erfolgen. Wenn an so einer Stelle eine Kreuzung ist,
dann kann die Angabe E<quot>Hauptstraße folgenE<quot> zusätzlich
angezeigt werden.
...
Unlimited Undo? Vielleicht auch persistent machen, so dass es auch
nach einem Programmneustart verfügbar ist? (Dann ist es vielleicht
nicht wirklich ein Undo, sondern mehr eine Historie, ähnlich wie bei
Gimp oder wie ein Browser-History)
...
-> 2012-07: hmmm, wozu brauche ich Portrait/Landscape überhaupt?
   Scheint mir in keiner Weise sinnvoll zu sein. Vielleicht nur, wenn
   das Programm auf einem mobilen drehbaren Gerät laufen würde. Aber
   würde nicht der X-Server das Drehen übernehmen? Ich habe gerade die
   Menüpunkte inaktiv gemacht, aber den Code nicht entfernt...
...
Das könnte man vielleicht mit einem HList machen, siehe
-splitter-Option.
...
Oder sollte ich lieber die andere Implementation basierend auf
BBBikeSuggest (siehe babybike) verwenden?
...
-> Siehe lib/get_and_store_synop und lib/parse_synop: ist es das
   schon? Wird es auch schon erfolgreich verwendet (e.g. osm-Daten,
   vielleicht auch optional für BBBike-Daten)
...
zu laufen. Zum Testen könnte ich einen blockierenden HTTP-Server
simulieren und die zu fetchende URL ändern.
...
kann, aber weder Subject noch Body. (-> Wie sieht es bei firefox aus,
existiert print() dort auch nicht?)
...
Bike+Ride könnte auch beim CGI interessant sein, falls die Kooperation
mit VMZ klappt. Dort könnte ich wahrscheinlich eine echte ÖPNV-Suche
triggern.
...
bei Windows (oder allgemein: bei ClickToFocus) ist es unangenehm, dass
neue Fenster den Focus klauen. Allerdings ist es ohne Maus eher
angenehm, dass neue Fenster den Focus bekommen. Wie soll man also
verfahren? -> Ich habe Cycle-Windows mit CTRL-TAB implementiert.
Vielleicht reicht das schon? Allerdings benutze ich es nie
(vergessen!), obwohl ich es häufig brauche.

Siehe auch das Item zu focusFollowsMouse in diesem Dokument.
...
Tk Widget) -> schon korrigiert??? -> (2012-07) Häh?
...
=item Farbeditor (H1)
 ...
 - Farbeditor: background setzen sollte vielleicht Canvas-bg ignorieren bzw. eigener Auswahlpunkt für Canvas-bg. Vielleicht *alle* Farben änderbar machen --- aber dann muss man die Änderungen auch abspeichern können

=item weiter

 - Straßennamen: zeichnen on demand? besserer Algorithmus? TIP mit
   -angle ausprobieren? (schwer) -> aber mit -lazy habe ich schon "on
   demand". Reicht das (abgesehen davon, dass -lazy noch Bugs hat)
...
2012-07: stimmt das noch immer mit den zusätzlichen
enter/leave-Opcodes. Vielleicht mal einen Benchmark machen und
schauen, ob sich das tatsächlich auf die Zeit auswirkt? Die redefine
warnings können (mittlerweile?) per "no warnings" ausgeschaltet
werden. Evtl. auch nur, wenn man die Syntax *CONST = sub () {...}
verwendet.
...
turn on lazy mode, draw E<quot>data/zebrastreifenE<quot> as a point
layer).
...
-> Hmmm, benutze ich irgendwo irgendwie focus_policy=follow? Auf mosor
   jedenfalls nicht, der Default ist click, und follow hat fast keinen
   Effekt. Check, und vielleicht allen Code, der sich damit
   beschäftigt, entfernen?
...
-> Gibt es diese Seiten noch? Und: auf welche Weise sollte das
...
-> Check if this URL still exists, and if it is usable now, and
   whether they are using osm data or what else.
...
-> I deactivated The link already in 2009. Check again if the site is
   alive, and if not, then remove the outcommented code completely.
...
-> Route::Descr hat mittlerweile M(...) support! Bloggen!
...
E<quot>(A -) BE<quot> zu E<quot>-> BE<quot> optimieren (bei den
Routenlabels). Ein konkretes Beispiel finden?
