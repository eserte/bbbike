...
Normal-Mode zeigen. Außerdem sollten einige Direktiven auch suchbar
sein.
...
Try Win32::GUI::SplashScreen as splash alternative.
...
aus Bundle/BBBike_windist.pm erzeugen kann.
...
S- und U-Bahnlinien mit den neuen Attributen Em und Cu versehen. Diese
sollten dann dafür verwendet werden, um das Layering anzupassen.
Außerdem U-Bahntunnelstrecken als solche markieren.
...
Nochmal sichergehen: (leider existiert dieser Link nicht mehr)
L<http://www.berlin-mitte.de/index_6819_de.html>
...
automatisch als öffentliche Fragezeichen erzeugt werden
...
Die öffentlichen Fragezeichen können bleiben wie bislang; evtl.
könnten die last_checked/next_check-Datensätze automatisch aufgenommen
werden.

Im Editier-Modus muss zusätzlich sichtbar sein:
- die add_fragezeichen-Datensätze
- die XXX-Datensätze (vielleicht zwei Varianten hier erzeugen?)

-> Pragmatischer Ansatz: im edit-mode werden einfach noch die
fragezeichen-Strecken zusätzlich zu fragezeichen-orig angezeigt.

*** Neuer Versuch ***

Es gibt drei fragezeichen-Dateien am Ende:

 - fragezeichen-public (weiterhin nur E<quot>fragezeichenE<quot>
   genannt) 
 - fragezeichen-private-vorort (für alles, was E<quot>vor OrtE<quot>
   recherchiert werden kann; falls es eine
   nextcheck/lastcheck-Direktive gibt, wird diese beachtet)
 - fragezeichen-private-office (für alles, was im Internet oder
...
Ungefähre Einordnung der jetzigen Daten:
 ...
 - section private Fragezeichen: bleibt so
 - Aufnahme in DB fraglich: dieser Abschnitt sollte entfernt werden
 - Isolierte Einträge: fragezeichen-private-vorort
 - öffentliche Fragezeichen: bleibt so
 - XXX -> fragezeichen-private-vorort
 - add_fragezeichen -> fragezeichen-public
 - weiterhin wird eine neue Direktive für fragezeichen-private-office
   benötigt 

Übrigens: vor/am Ort -> on-scene, on-site
...
Das Strippen des Bezirk-Parts im Straßennamen: nicht für fragezeichen,
da hier oft ein ergänzender Kommentar steht.
...
werden. Siehe auch nächstes Item.

=item * (H5)

Es sollte eine weitere Abgrenzung bei den Kategorien 1 (Einbahnstraße)
und 2 (gesperrte Straße) geben, und zwar Einbahnstraßen/gesperrte
Straßen, die man mit Schrittgeschwindigkeit passieren kann.
Darstellung wäre also als Einbahnstraße, vielleicht mit einem
speziellen Symbol dazu, und im Routing würde es als q4 angesehen
werden. Dann könnte ich vielleicht auch die E<quot>1sE<quot>-Kategorie
entfernen.

In der Routingliste (bbbike.cgi und Perl/Tk) sollte ein entsprechender
Text (E<quot>gegen die Einbahnstraße schiebenE<quot>) auftauchen.

Ganz wichtig: Straßen wie B96a/Adlergestell, die wirklich nicht
passierbar sind, sollen es auch bleiben! Be careful...
...
Zwischen Rudow und Lichtenrade wird man nicht über Großziethen
geroutet. Lösung: Luftlinie zwischen Start-(Via)-Ziel legen, falls
diese die Berliner Grenze kreuzt, muss der Scope von city auf region
erhöht werden.
...
referenziert. Daraus kann man zumindest die Länge herausholen (die
...
Missing stipples for Cemetery etc.

=item * (M5)

Ich sehe 404 für /cgi-bin/radroute.html und .../help.html mit Firefox
2.0.0.3 unter Windows. Wird <base> hier nicht korrekt erkannt? ->
confirmed: ich sehe Probleme mit Firefox 1.0.7, 1.5, 1.5.0.1,
1.5.0.2, 1.5.0.3, 1.5.0.4, 1.5.0.5, 1.5.0.6, 1.5.0.7, 1.5.0.9, 2.0,
2.0.0.1, 2.0.0.2, 2.0.0.3 (verschiedene OS) und einigen Java- und
anderen Bots. -> Workaround durch absolute URLs, aber vielleicht gibt
...
Tcl version is too slow --- get rid of the E<quot>calculate and show
all crossingsE<quot>

=item * (H1)

Java version does not compile anymore with Java 1.5
...
copy-cvs-to-stick-Makeregel: cache-Verzeichnis sollte nicht auf dem
Stick existieren
