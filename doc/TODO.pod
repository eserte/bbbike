...
auf usb-stick kopieren! -> make-target dafür schreiben
...
install.pl: Is completely broken now and should made working for
current GNOME/KDE versions.

Also: the toplevel resizing seems to be broken in kwin.

Windows support is not really necessary anymore, as we have the ISS
installer.
...
Maybe try Pod::ProjectDocs for pod2html conversion. But not the 0.31
version, as it cannot handle umlauts and entities correctly. See
