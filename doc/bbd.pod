...
The next block or line is just a temporary feature. Typically this
directive is accompanied by a C<next_check> directive to specify the
end date, so systems like "fragezeichen-nextcheck" can include this
feature in their lists.

Earlier versions of this document suggested to put the date
information into the Name field of the data line, preferred in a
ISO-date similar notation (e.g. YYYY-MM-DD - YYYY-MM-DD for a starting
and an end date). This may still be done additionally to the
C<next_check> directive, but is not evaluated by automatic scripts.
...
use just C<XXX> in conjunction with C<next_check> or C<last_checked>,
...
example, "culdesac" entries in F<comments_misc-orig> are skipped when
...
An (official) URL referring to more information.
...
One way street (read E<quot>one softE<quot>), but not strictly for
...
handicap of class C<q4> for the wrong direction.).

=item * C<1h>:

One way street 8read E<quot>one hardE<quot>), and there's no
