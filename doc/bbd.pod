...
=item C<category_dash.I<category>: I<x, y, ...>>

Define the dash pattern for line items with the category I<category>.

=item C<category_color.I<category>: I<color>>

Define the color for line items with the category I<category>.
I<color> should be in the form C<#rrggbb>.

=item C<category_image.I<category>: I<image_name>>
...
=item C<category_stipple.I<category>: I<image_name>>

Define an stipple (which is usually an xbm image) for area items.
...
This can be used if stacking information has to be given using
ordering in the data file, but bbbike should really have a layer_stack
attribute.

It can also be used for the cyclepath layer, because of suboptimal
rendering in points with large angles (more than 45° or so). Here the
layer item has to be broken into two items to fix the rendering
problem.

=item C<XXX_indoor: I<why>>

The unclear information can only be researched E<quot>indoorE<quot>,
but usually not on-site.

=item C<XXX_outdoor: I<why>>

The unclear information can only be researched on-site. Usually this
is the same as C<XXX>.
