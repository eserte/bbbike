...
for VIZ traffic reports), C<bvg2021> (BVG traffic reports: prefixed by
the affected line, then a C<#>, then the id), and scheme-less (a real
VIZ id). Sometimes also other forms of pseudo ids or even URLs are
used here.
