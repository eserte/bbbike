...
Hauptfenster öffnet sich. Das Fenster sollte möglichst groß sein,
eventuelle Taskleisten aber nicht verdecken.

Das Straßennetz erscheint und weitere Layer: U- und S-Bahn (sowohl
Linien als auch Bahnhöfe mit Benamung), Flächen (z.B. Parks), Gewässer
und Ampeln. Zentriert wird auf das Brandenburger Tor. Falls die
Datenbasis nicht zu alt ist, ist es sehr wahrscheinlich, dass
temporäre Sperrungen erscheinen (mit Baustellen- oder Kalendarsymbol).

Straßenauswahlliste erscheint. Die A-Z-Leiste sollte vollständig sein
...
Texte in der Anwendung: die Schriftgröße sollte der Schriftgröße der
...
I<Windows>: Das Programm wird mit C<wperl> gestartet und hinterlässt
damit nichts im Terminalfenster und öffnet auch kein neues
Terminalfenster.

=head3 Im Auswahlfenster einen weiter hinten stehenden Buchstaben
wählen

In vernünftiger Zeit sollte die erste Straße mit dem gewählten
...
=head3 Eintippen eines Straßennamens
...
Die Dudenstraße wird sichtbar, die Markierung blinkt einige Sekunden
rot und blau und die Startfahne wird auf die Mitte der Dudenstraße
positioniert.

=head3 Eingabefeld löschen und weitere Straße eintippen

E<quot>seumestrE<quot> eintippen. Der Straßenname wird in der Listbox
angezeigt.

=head3 Return drücken

Die Karte wird so gescrollt, dass die Seumestraße sichtbar wird. Die
Markierung blinkt für einige Sekunden.

=head3 Auf E<quot>ZielE<quot> klicken

Die Zielfahne wird auf die Mitte der Seumestraße positioniert (B<NYI>:
wird erst zum Schluss dieser Aktion durchgeführt).

Der Mauszeiger geht in den E<quot>SchlafE<quot>-Modus (Uhr oder
Z-Z-Z).

Die Route (rot gezeichnet) zwischen Dudenstraße und Seumestraße
...
Routenliste erscheint, ist komplett sichtbar und enthält eine
...
einem temporären Verzeichnis eingeben und mit E<quot>SaveE<quot> oder
E<quot>OkE<quot> bestätigen.

I<Windows>: Die erzeugte Datei sollte eine E<quot>.txtE<quot> besitzen
und mit einem üblichen Texteditor/-viewer zu öffnen und zu lesen sein.

I<X11>: Die Datei sollte mit einem Texteditor/-viewer zu öffnen und zu
lesen sein. Es wird keine Defaultextension erzwungen.

Umlaute sollten möglichst korrekt dargestellt sein (noch: als
...
GPS-Gerät erscheinen. Falls eine Fehlermeldung auftritt, muss in
GPS-Einstellungen u.U. der serielle/USB-Device geändert werden.
...
länger dauern) und das erzeugte PDF-Dokument wird angezeigt. Fenster
...
E<quot>SubjectE<quot>, E<quot>zusätzlicher TextE<quot> erscheint.
E<quot>Mail anE<quot> mit der eigenen Adresse füllen.
...
zusätzlichen Text eingeben. Mit E<quot>OKE<quot> bestätigen.

I<Windows>: Typischerweise wird ein externes Mailprogramm geöffnet, auf
dem man die Angaben nochmal sieht und bestätigen kann.

B<KNOWN ISSUE>: Der Routentext ist tabellarisch aufgebaut, aber
Mailprogramme formatieren Text typischerweise in einen
Proportionalfont.

Die Mail sollte als Text-E-Mail ankommen. Vor dem zusätzlichen Text
sollte sich die Routenliste befinden.

=head3 Auf den Checkbutton E<quot>Straßennamen an der RouteE<quot>
klicken

In der Karte erscheint an jeder Kreuzung mit wechselndem Namen der
Straßenname. Straßennamen sollten sich nicht überlagern. Die
Straßennamen sollten oberhalb der Route erscheinen.

=head3 Auf den Checkbutton E<quot>km-AngabenE<quot> klicken

Zusätzlich zu den Straßennamen erscheinen Kilometerangaben in
Klammern. Die oben genannten Bedingungen sollten auch hier gelten.

=head3 Auf den Checkbutton E<quot>KompaktE<quot> klicken

Die Routenliste wird kleiner, indem einige Straßen zusammengefasst
werden (dort, wo nicht abgebogen werden muss). Auch auf der Karte
werden die zusammengefassten Straßen nur noch mit der ersten Straße
markiert.
...
=head3 Das Fenster mit der E<quot>Aktuellen RouteE<quot> schließen

=head3 Mit der rechten Maustaste auf der Karte klicken (Mac: Apfel-Maustaste)

Ein Popup-Menü erscheint. B<NOTE>: bei einigen GUI-Systemen (X11) kann
es notwendig sein, die Maustaste gedrückt zu halten.

=head3 Im Menü E<quot>Suche wiederholenE<quot> auswählen

Das Fenster mit der E<quot>Aktuellen RouteE<quot> erscheint wieder,
sonst passiert scheinbar nichts.

=head3 Popup-Menü wieder aktivieren, E<quot>RückwegE<quot> auswählen

Der Mauszeiger geht wieder in den Schlafmodus.

Start- und Zielflagge werden auf Karte vertauscht.

Möglicherweise ändert sich die Route.

=head3 Popup-Menü wieder aktivieren, E<quot>Gesamte Route
löschenE<quot> auswählen
...
=head3 Den Mauszeiger auf eine Straße bewegen

Nach einiger Zeit erscheint ein Balloon mit dem Namen der Straße.

Zwischenzeitlich einen anderen Layer einschalten (z.B. Qualität mit
"Shift-Q"): weitere Eigenschaften der neuen Layer werden ebenfalls im
Balloon angezeigt.

=head3 Mit dem linken Mauszeiger auf eine Straße klicken

Die Startflagge wird gesetzt.

=head3 Mit dem linken Mauszeiger auf eine weiter entfernte Straße
klicken
...
=head3 Linke Maustaste auf der Karte gedrückt halten und bewegen

Der Mauszeiger ändert sich zu einer Hand (I<X11>) oder zu einem Kreuz
(I<Windows>) und die Karte wird (langsam)
in der gleichen Richtung gescrollt.

=head3 Mittlere Maustaste (Mac: Alt+Maustaste) auf der Karte gedrückt
...
=head3 Linke Maustaste über den Pfeilen der Windrose etwas länger
gedrückt halten

Die Karte scrollt, solange die Maustaste gedrückt bleibt.

=head3 Scrollbalken-Pfeile (vertikal und horizontal) anklicken

Die Karte wird in der angeklickten Richtung gescrollt.

=head3 Scrollrad nach oben und unten betätigen
...
=head3 Shift drücken und dabei das Scrollrad betätigen
...
=head3 Die beiden zusätzlichen Fenster schließen

=head3 Den Button mit der kleinen Berlin-Karte anklicken

Der Button wird mit einem roten Indikator markiert.

Es wird ein kleineres Fenster in der rechten unteren Ecke geöffnet.
Das Fenster zeigt die Umrisse von Berlin, einige Berliner Bezirke und
Ortsteile, einige Orte im Umland, stilisierte Gewässer, ein Rechteck
für den sichtbaren Kartenbereich und die aktuelle Route als rote
Linie.

=head3 Auf E<quot>-E<quot> klicken

Die Karte wird neu gezeichnet (Progressbar in Aktion). Die Grenzen von
Berlin erscheinen nun kleiner, ebenso das Rechteck, dafür erscheinen
mehr Orte im Umland.
...
=head3 In der Übersichtskarte auf E<quot>TegelE<quot> klicken

Das Rechteck in der Übersichtskarte wurde nun auch nach Tegel
verschoben; in der großen Karte ist nun Tegel sichtbar.

=head3 Mit gedrückter linker Maustaste von Tegel nach Köpenick ziehen

Das Rechteck in der Übersichskarte wird entsprechend der Bewegung
verschoben; die große Karte ebenso. Eine Verzögerung ist zu erwarten,
sollte sich aber im Rahmen halten.

=head3 Eine Route durch eine temporäre Sperrung erzeugen

Die Route wird erzeugt und die Leiste "Mögliche temporäre Sperrungen
auf der Route" erscheint.

=head3 Auf "Anzeigen" klicken

Es erscheint ein Fenster mit einer oder mehreren temporären Sperrungen.

=head3 Die Checkbox "Aktivieren" aller Sperrungen auswählen und "Ausgewählte umfahren" klicken

Es wird eine neue Route berechnet, die die aktivierten temporären
Sperrungen umfährt. Das Fenster mit den "Aktuellen Sperrungen" bleubt
stehen, aber die ausgewählten Sperrungen befinden sich nun unterhalb
der Überschrift "Bereits behandelte Sperrungen.

=head3 Fenster "Aktuelle Sperrungen" schließen

=head2 Menüfunktionen

Voraussetzung: eine Routesuche wurde durchgeführt und die Route ist
noch markiert.

=head3 Datei > Speichern

Ein Dateiselektor öffnet sich im Verzeichnis F<.../.bbbike/route>.

Man wählt nun einen Namen aus, z.B. F<test> und bestätigt.

=head3 Datei > Öffnen

Ein Dateiselektor öffnet sich im Verzeichnis F<.../.bbbike/route>. Der
...
=head3 Datei > zuletzt geöffnete Dateien

Die Datei F<.../.bbbike/route/test.bbr> sollte am Anfang des
kaskadierenden Menüs erscheinen. Das Menü durch Klicken auf eine
Nicht-Menü-Fläche schließen.
...
Ein Dateiselektor öffnet sich, z.B. mit dem Heimatverzeichnis des
Nutzers. Ein (temporäres) Verzeichnis auswählen und dort F<test>
eingeben und bestätigen.

Die geschriebene Datei sollte F<test.ps> heißen und kann mit einem
Postscript-Viewer (z.B. ghostview) angesehen werden.
...
I<Windows>: Diese Funktion benötigt C<ps2pdf> aus der
...
Die geschriebene Datei sollte F<test.pdf> heißen und kann mit einem
...
Die geschriebene Datei sollte F<test.png> heißen und kann mit einem
...
I<Unix>: Diese Funktion benötigt Nicht-Standard-Perl-Module, die
möglicherweise nicht verfügbar sind.

Beim Speichern den Dateinamen F<test> verwenden. Die erzeugte Datei
sollte F<test.gpx> heißen, der Inhalt nach XML ausschauen.
...
Beim Speichern den Dateinamen F<test> verwenden. Die erzeugte Datei
sollte F<test.bbd> heißen.

=head3 Kartenebenen > Zusätzliche Kartenebenen > Straßen-Layer
zeichnen

Auf den Dateiselektor-Button klicken und die Datei F<test.bbd>
auswählen. 

=head3 Im Straßen-Layer-Dialog OK anklicken

Ein neuer Layer wird gezeichnet, der genau der aktuellen Route
entspricht und deshalb nicht sichtbar ist.

=head3 Route > Route löschen > Gesamte Route löschen
...
=head3 Kartenebenen > Zusätzliche Kartenebenen > Layer löschen
...
B<NOTE>: Erzeugung dauert etwas länger.

Die erzeugte Datei sollte F<test.pdf> heißen. Sie kann mit
...
mit der Straßenkarte und einer roten Route; die zweite Seite mit der
tabellarischen Routenbeschreibung.

=head3 Datei > Volltextsuche

Ein Fenster mit einem Eingabefeld für E<quot>NachE<quot> und weiteren
Elementen erscheint.

=head3 In das Nach:-Eingabefeld E<quot>dudenstrE<quot> eintippen und
auf den E<quot>SuchenE<quot>-Knopf drücken

In der Ergebnisliste erscheinen einige Einträge, zumindest
E<quot>Dudenstr.E<quot>, etwas eingerückt unter der Überschrift
E<quot>Straßen und Plätze in Berlin (strassen)E<quot>.

=head3 Doppelklick auf E<quot>Dudenstr.E<quot>

Der Kartenausschnitt wird so bewegt, dass die Dudenstraße sichtbar
wird. Eine Markierung blinkt einige Sekunden lang.

=head3 Mauszeiger über eine Straße positionieren und auf
E<quot>IE<quot> drücken.

I<Windows>: da das Hauptfenster möglicherweise nicht den Fokus hat,
passiert nichts beim Drücken. Alternativ auf das i-Icon in der
Symbolleiste drücken.

Es erscheint ein Fenster mit dem Titel E<quot>InformationE<quot>.

Der Straßenname, die Länge der Straße und verschiedene Koordinatenformen
...
Verfügung stehen (z.B. Zeiten für Sonnenaufgang/-untergang).

=head3 Den E<quot>MapserverE<quot>-Link anklicken

Es wird ein Browser geöffnet und die Mapserver-Seite mit dem
aktuellen BBBike-Kartenausschnitt (ungefähr) geladen.

=head3 Browser-Fenster schließen oder symbolisieren
...
Es wird ein Browser geöffnet und die BBBike-WWW-Startseite wird
geladen. Der selbst geschriebene Zielnamentext wurde für den Zielpunkt
übernommen.
...
=head3 Den Knopf E<quot>SchließenE<quot> im Informations-Fenster
auswählen

Das Fenster wird geschlossen.

=head3 Einstellungen > Schriftart ändern

I<Unix, MacOSX>: L<Tk::FontDialog> wird für die Schriftartenänderungen
benötigt, ist aber nicht standardmäßig bei Tk dabei.

Ein Zeichensatzdialog erscheint. Die aktuelle Schriftfamilie und
-größe sollte selektiert sein (unter X11 typischerweise
E<quot>HelveticaE<quot> und E<quot>10E<quot>, kann aber differieren).

=head3 Eine andere Schriftgröße, z.B. E<quot>8E<quot> wählen und OK
drücken

Alle Texte im Menü, im Auswahlfenster sollten sich entsprechend
ändern.

Texte, die sich auf der Karte befinden, bleiben unverändert.

Native Texte (z.B. die Menüleiste unter Windows) bleibt u.U. auch
unverändert.

=head3 Klick auf das Hilfe-Symbol (mit dem Fragezeichen)

Es öffnet sich ein Fenster mit der BBBike-Dokumentation, bevorzugt als
internes BBBike-Fenster (Tk-Pod-Viewer), aber zumindest erscheint die
Dokumentation im WWW-Browser.

=head3 Einstellungen > Daten-Update über das Internet

B<NOTE>: Achtung: dieser Menüpunkt darf nicht genutzt werden, wenn die
Dateien im F<data>-Verzeichnis über git up-to-date gehalten werden,
oder wenn Kartendaten selbst geändert wurden und nicht verloren gehen
dürfen.

B<NOTE>: Dieser Test sollte zuletzt durchgeführt werden.

Ein Dialog erscheint, der den Benutzer über das Update informiert.
...
Straßennetz aktualisiert, falls sich auf dem Server neuere Daten
befinden.
...
Unter Unix würde sofort angefangen werden zu drucken (vielleicht).

=head3 Update über das Internet testen
...
=item Alternative 1
    ...
    * Entweder eine alte Distribution in /tmp auspacken oder einen
      älteren git-Stand in /tmp auschecken

    * Falls von git: das .git-Verzeichnis und data/Makefile löschen.
      Außerdem ein data/.modified besorgen
    ...
    * Nach dem Laden gibt es ein Reload der neuen Straßen.
...
=item Alternative 2
    ...
    * Folgende Kommandos ausführen (vorherige und jetzige
      BBBike-Version ändern falls notwendig):
    ...
    * Update-Menüpunkt testen

    * Wurde alles wie erwartet geladen? Wurden alle Layer automatisch
      aktualisiert?

    * Sehen die Einträge in access.log auf dem Server wie erwartet
...
Die erzeugte Datei sollte F<test.fig> heißen. Sie kann beispielsweise
...
B<NOTE>: Unterscheidet sich diese Datei von der alten überhaupt?

=head3 Das Menü Plugins > Alle Plugins anzeigen auswählen

Es sollten ungefähr fünf Plugins in der Liste erscheinen (exakte Zahl
ist nicht wichtig).

=head3 Das Plugin E<quot>BBBikeSalesmanE<quot> durch Anklicken der
Checkbox laden und auf E<quot>SchließenE<quot> klicken
...
Für jede Station wird eine Fahne gesetzt; außerdem erhöht sich die
...
E<quot>AbbrechenE<quot> drücken.

Die Berechnung sollte abgebrochen werden. Gegebenfalls wird die beste
bislang gefundene Route eingezeichnet. B<KNOWN ISSUE>: unter Umständen
wird der Progressbar nicht zurückgesetzt, außerdem können
Fehlermeldungen erscheinen.

=head2 Manuelle CGI-Tests

=head3 Layout der PDF-Routenliste prüfen
...
und auf die zweite Seite gehen. Alle vier Spalten sollten vollständig
sichtbar sein. Unter Umständen ist eine zweizeilige Zelle zu sehen:
...
Verwendung von Route::PDFCairo ohne Pango fließt die
Straßennamenspalte in die nächste Spalte über.)
