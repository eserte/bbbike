...
# Author: Slaven Rezic
#
# Copyright (C) 2003,2004,2005,2006,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024,2025,2026 Slaven Rezic. All rights reserved.
...
use strict;
use vars qw($lang $bbbike_url $bbbike_images $bbbike_script $bbbike_html $can_mapserver $mapserver_init_url $is_beta $fake_time $use_cgi_bin_layout);
...
sub teaser_sternfahrt_adfc {
    my $out_of_date = $today gt "20260607";
    if (!$out_of_date) {
	my $url = "https://berlin.adfc.de/sternfahrt";
	<<EOF
<div class="teaser" style="font-size:larger;"><a href="$url"><b>Sternfahrt ${year}</b></a> am 7. Juni $year</div>
EOF
    } else {
	();
    }
...
sub teaser_kreisfahrt_adfc {
    my $out_of_date = $today lt "20260905" || $today gt "20260919";
    if (!$out_of_date) {
	my $adfc_url    = "https://touren-termine.adfc.de/radveranstaltung/203240-adfckreisfahrt-2026";
	my $misc_dir = $use_cgi_bin_layout ? '/BBBike/misc' : '/bbbike/misc';
	my $kreisfahrt_img = "$misc_dir/kreisfahrt_anyyear/kreisfahrt_anyyear.png";
	<<EOF
<div class="teaser"><a style="text-decoration:none;" href="$adfc_url"><img src="$kreisfahrt_img" alt="ADFC-Kreisfahrt ${year}" border="0" style="position: relative; top: 3px;" /></a> am 19. September $year</div>
EOF
    } else {
	();
    }
...
sub teaser_halbmarathon {
    my $out_of_date = $today gt "20260329";
    if (!$out_of_date) {
	my $halbmarathon_map_url = 'https://viz.berlin.de/aktuelle-meldungen/verkehr-zum-45-berliner-halbmarathon-am-29-marz-2026/';
	if ($lang eq 'en') {
	    <<EOF
<div class="teaser" style="font-weight:bold">BERLIN HALF MARATHON on 29 March 2026<br/>
<a href="$halbmarathon_map_url">Map with blockings</a></div>
EOF
	} else {
	    <<EOF
<div class="teaser" style="font-weight:bold">Am 29. März 2026 findet der Berliner Halbmarathon statt.<br/>
