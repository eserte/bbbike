...
#
# $Id: bbbike.cgi,v 8.20 2006/08/27 22:26:02 eserte Exp $
# Author: Slaven Rezic
#
...
$VERSION = sprintf("%d.%02d", q$Revision: 8.20 $ =~ /(\d+)\.(\d+)/);
...
#XXX not yet:	    print "<td bgcolor=\"$color\">" . blind_image(1,1) . "</td>";
	...
	if ((defined $$nameref and $$nameref ne '') ||
	    ...
	    print qq{<a target="newstreetform" href="$bbbike_html/newstreetform.html?$qs">} . M("Diese Straße eintragen") . qq{</a><br>\n};
	    $no_td = 1;
	    $tryempty = 1;
	} elsif ($$tworef ne '') {
		...
		if ($report_nearest) {
		    print qq{<i>$strasse</i> } . M("ist nicht bekannt") . qq{ (<a target="newstreetform" href="$bbbike_html/newstreetform.html?$qs">} . M("diese Straße eintragen") . qq{</a>).<br>\n};
		} else {
...
EOF
    ...
    my $cgi_date = '$Date: 2006/08/27 22:26:02 $';
