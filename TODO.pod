...
Gespeicherte Routen machen beim Wiederladen Probleme: ein Verlängern
ist nicht möglich, Vias einfügen ist nicht möglich (es sei denn,
Start- und Zielpunkt fallen auf einen existierenden Punkt im Netz).

=item * (H3)

Optimize SRTProgress: only hide canvas if first progress drawing is
necessary. Adjust progress drawing to be specified in time intervals
(alternatively). This would reduce the need of hiding/displaying the
canvas in short or empty redraws.
...
ampel/andreaskr/etc. images. => done, but I get strange warnings, see
TODOs in BBBikeXS/t/base.t...
...
S-Bahn-Symbole sind suboptimal, evtl. GIFs stattdessen verwenden. Wenn
Platz ist, könnten auch die Bahnhöfe mit Labels versehen werden. Wenn
ich rotierte Fonts zeichnen kann, dann auch wichtige Straßen.
