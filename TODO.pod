...
Beim Ändern des Zeichensatzes mit E<quot>Schriftart ändernE<quot> wird
anscheinend der font E<quot>boldE<quot> nicht berücksichtigt. Gesehen:
im Infor-Fenster. Vielleicht betrifft das auch andere Schriftarten.
Möglicher Grund: vielleicht darf ich nicht fontCreate sagen, wenn ich
einen bestehenden Font ändern möchte? Oder vielleicht sollte
RefontTree auch Text-Tags ändern? Und Canvas-Items auch? 

Die A..Z-Leiste im Auswahlfenster verwendet anscheinend keinen Font
aus %font (und wird auch bei E<quot>Schriftart ändernE<quot> nicht
geändert. Siehe vorherigen Absatz.

=item * (M5)

Bei der Verwendung von -public sollte ein andere Konfigurationsdatei
zum Speichern verwendet werden, um versehentliches Speichern einer
Konfiguration beim Testen zu vermeiden. Ebenso sollte es bei -public
eine Möglichkeit geben, eine Konfiguration wissentlich zum Testen zu
laden. (-> doc/tests.pod)

=item * (H5)

Besserer Plugin-Dialog. Plugins sollten aus der Plugin-Liste permanent
ein/ausgeschaltet werden können. Ebenso sollten die Plugin-Argumente
in diesem Dialog angezeigt und editiert werden können. Plugin-Dialoge
von anderen Anwendungen (gimp, gaim, mozilla, firefox) anschauen.

=item * (M5)

Check: ist die Reihenfolge der Nachbarpunktselektion beim
nachträglichen Via-Einfügen egal oder gibt es hier einen Bug?
...
Umlauts get lost in fvwm2 titlebars (Debian, fvwm2.4.x and fvwm2.5.x,
Tk800.024). No problem on FreeBSD with fvwm2.4.15. On Mac and
Tk804.027, umlauts are shown as unicode escape sequences \x{....}

Maybe some utf-8 problem? Maybe newer fvwm2 respect the locale?
...
Prüfen, ob Font::Metrics::Helvetica in der winbindist dabei ist und
tatsächlich bei der PDF-Erzeugung benötigt wird.

=item * (*M3)

Prüfen, ob String::Approx/String::Similarity in der winbindist dabei
ist und ob das Modul irgendwo verwendet wird (approximative
Volltextsuche?). Auch: in doc/tests.pod aufnehmen!

=item * (*M3)

Prüfen, ob das Problem des Handlungsreisenden mit der winbindist
funktioniert. Auch: in doc/tests.pod aufnehmen!

=item * (H4)

Nach dem Auswählen einer Aktion im Popup-Menü ist der Fokus nicht mehr
auf dem Fenster. Womöglich geht der Fokus auch bei anderen Aktionen
verloren.
...
lassen. -> DONE, check missing!

=item * (H2)

Make some research about alternative perl distribution on windows.
Unfortunately I seem to be unable to compile Tk with Strawberry Perl.
...
New BBBike Plugin for Wikimapia:

Die URLs sehen so aus:

L<http://wikimapia.org/s/#y=52521235&x=13362122&z=15&l=5&m=a>

z ist die Zoomstufe, wahrscheinlich wie bei googlemaps
