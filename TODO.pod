...
=item * Check everything in (almost only data is missing)

=item * Write Changes/Release notes (well... take it easy)

=item * Add some of the suggested modules to the win32 distribution,
update e.g. the perl gps distro

=item * a -public check on Win32, Linux and FreeBSD

=item * perl/tk: E<quot>kompaktE<quot> in show_route_strnames
funktioniert nicht richtig

=item * try the inno setup on windows (if it does not work: backup the
CHANGES text)
 ...
 * It would be nice to include Tk::Stderr into the Windist
 ...
 * Update -> E<quot>Daten-UpdateE<quot> benennen
...
werden. => Ist das noch immer problematisch? Ich dachte, ich hätte im
CGI zusätzliche Logik eingeführt, um dieses Problem zu umgehen. Eine
ähnliche Logik fehlt vielleicht noch in BBBikeRouting.pm => Nein, es
geht noch immer nicht. Test: Suche nach Treptower Hafen.
...
Überprüfen: wird womöglich eine cooked-Version beim Erstellen des
PDF-Straßennetzes verwendet?
