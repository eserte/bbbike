...
selection. Otherwise, if FontDialog is not available, the chosen fonts
should be made immediately visible using set_fonts().
...
Make sure that Tahoma 8 is used as default font on windows. Do I need
to use a larger fonts if user's font settings are different?
