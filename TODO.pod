...
last_loaded_streets muss komplizierter werden und den gesamten State
beim Zeichnen der Datei enthalten (z.b. die Aufrufparameter von
plot())
...
BBBikeXS, fast_plot_point: use main::get_symbol_scale() to get name of
ampel/andreaskr/etc. images.
...
Speichern von Grafiken: Extension erzwingen (oder wird das bereits
gemacht?), E<Uuml>berschreiben testen und verhindern (oder wird das
bereits gemacht?)

=item * (??)

evtl. mit fork eine Warteschleife implementieren. Wenn es nicht mit Tk
(threads!) geht, dann vielleicht mit X11::Protocol oder mit xterm? XXX
Warteschleife?
...
Stimmt das noch immer, auch mit Http.pm?
...
=item Ideensammlung für schrittweise landstrassen-orig-Auflösung in Brandenburg
  ...
  0. Änderungen für Variante 1 und 2
    - Es wird in ein neues landstrassen-orig mit Hafas-Koordinatensystem
      gewandelt: DONE!
    - BASE: landstrassen-orig => H: DONE!
    - alle Punkte aus landstrassen-orig, die sich in anderen bbd-Dateien
      befinden, müssen auch umgewandelt werden: DONE!
...
bbbike.cgi (Kopfsteinpflaster vermeiden, rennraudtauglich, Custom).

=item * (E1)

Show all plugins: Change the dialog to select first the plugin and
then on a button E<quot>Load pluginE<quot> (multiple selection
possible). Add a cancel button. Double-1 is a shortcut for loading the
plugin.
...
Zusammenstellung von Radrouten in Brandenburg:
L<http://www.reiseland-brandenburg.de/pages/rad.html>
...
=over
 ...
 alle Brücken aus plaetze => brunnels überführen
...
misc/fragezeichen nach data migrieren. Alle Skripte/Module anpassen
(MANIFEST, CVS, RCS-rename, in insert_points aufnehmen,
data_corrected/fragezeichen aus bbbike.pl entfernen ...)

=item * (M1)

Convert the data/Makefile to data/Makefile.PL or similar (to solve
problems like for loops etc.).
...
all and comment types (partially done in bbbike).
...
=head1 Distribution/Installation
