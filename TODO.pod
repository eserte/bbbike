...
=head1 Unsorted after Win98 session

 * option editor does not fit into 800x600 screen --- using scrollable pane?
 * Drucken: if the PDF backend is chosen, then Druckeinstellungen have no effect. Maybe it
    would be nice to support at least some of them (legend, maybe b/w)
 * Overview-Map: always segfault
 * Archive::Zip-Teil fuer cgi/bbbike-data.zip schreiben. Test fuer dieses Skript schreiben.
...
Check: Straßen-Editor sollte bei E<quot>add newE<quot> alle
bestehenden -orig-Dateien anzeigen.

=item * (E3)

Nicht mehr genutzte Editierfunktionen in ein Cascade mit dem Titel
E<quot>unsupportedE<quot> verschieben.

=item * (E3)

"Blankenburger See Uckermark" should only be "Blankenburger See" ---
remove everything after the Pipe in the map output. (Is this already
done for orte?)

=item * (M2)

Möglichkeit des Splittens von Flächen schaffen. Schließlich den
Katzengraben (zwischen Baumgarteninsel und Altstadt) schaffen.

=item * (E2)

Make canvas tags clickable by URL (e.g. new menu item in E<quot>Zusätzliche LayerE<quot>):

    $c->bind("L3-fg", "<1>" => sub { ($url) = grep { $_ } map { m{(http://\S+)}; $1 } $c->gettags("current"); require WWWBrowser; WWWBrowser::start_browser($url) })

=item * (H3)

Unterstützung für E<quot>Persönliche WegeE<quot> (oder user-defined
layers). Zwei Typen: routingfähig (automatisch per
E<quot>add...E<quot> in das Straßennetz eingebunden) oder
nicht-routingfähig. Editiermöglichkeiten für diesen Layer (mit
Verwendung von datachange.log). Möglichkeit der Veröffentlichung.

=item * (H3)

Ich sollte GPX als Ein/Ausgabeformat unterstützen: L<http://www.topografix.com/gpx_resources.asp>

=item * (H3)

mount E<amp> comments_mount virtuell integrieren. Neue Symbole für
Steigungen verwenden:
...
The -lazy option seems to cause a big memory leak. (Hmmm, I cannot
reproduce this one anymore...)
...
To speed up the update, just upload .modified from server
first and compare the dates exclusively by using .modified!
...
=> Wo verwenden: overview-Fenster, overview-Button oder im CanvasMap
...
optional auch für X11, notwendig fuer Tk804)
...
Potsdam ist größer geworden (Golm ...). Potsdam.coords.data erweitern,
vielleicht auch zwischen den Potsdamer Ortsteilen unterscheiden?
...
relation_gps-orig aus insert_points herausnehmen? Ueberdenken...
...
kommt ins MANIFEST.addtocvs, aber nicht ins MANIFEST.
...
data_corrected => data-Transition

Problems: Nord-Rügen. Stützpunkte in Hiddensee und Sassnitz einfügen.
...
Recherchieren! => Jan hat etwas gefunden (Aldi-Karten...)
...
Bezirke, also "Kreuzberg, Bahnhofstr."?

=item * (E3)

/robots.txt einrichten, um das Indexieren von temporären Dateien bei
bbbike und mapserver zu verhindern.
...
Info page: link to screenshot html page instead of single screenshot
image. (?)
...
Why are the E<quot>NE<quot> streets and airports colored black? Wrong
mapserver/brb/brb-ipaq.map?
...
Route wird bei bbbikedraw.t nicht gezeichnet?
...
Falls die Routenliste nur aus einem Punkt besteht (Beispiel:
?center=;interactive=Grafik%20zeichnen;imagetype=png;coords=11824%2C8995;startname=Hermannplatz;zielname=Karl-Marx-Str.;geometry=640x480;draw=str;draw=title;scope=),
dann bbox automatisch auf eine Mindestgröße vergrößern.
...
Brushes für die Linienbreite vielleicht rund statt quadratisch machen?
...
transparent - blau - transparent? Ebenfalls zu übelegen bei
BBBikeDraw::PDF.

=item * (M4)

bbbikedraw.t-Test: Werden Routennamen gezeichent? Wenn nein, dann
einschalten!
...
README für MacOSX erneuern.

=item * (M2)

HOWTO zum eigenen Editieren der Daten, zum Erzeugen von neuen Layern
etc.
