...
# XXX Leave funktioniert nur f�r max. 12 Stunden (testen!)
...
sub enter_alarm {
	...
	$top->messageBox(
			 -message => M"Keine Fahrzeit �bergeben",
			 -icon => 'error',
			 -type => 'OK',
			);
    } else {
	...
	if (defined $args{-location} && eval { require Astro::Sunrise; Astro::Sunrise->VERSION(0.85); 1 }) {
		...
		(-variable => \$sunset_choice,
		 -options => [["" => ""],
			      ...
			      [M("Ende der b�rgerl. D�mmerung") => $sunset_civil],
			     ],
		 -command => sub {
...
sub show_all {
	...
	while(my($k,$v) = each %$pids) {
	    ...
	    my $state = "unknown";
	    if ($host eq $this_host) {
		$state = (kill(0 => $pid) ? M("l�uft") : M("l�uft nicht"));
	    }
	    push @l, $state;
...
=head1 TODO
    ...
    sollte ich evtl. verwenden f�r die Liste der Alarme:
