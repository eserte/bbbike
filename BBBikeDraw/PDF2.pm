...
#
# $Id: PDF2.pm,v 1.9 2007/07/28 20:59:18 eserte Exp $
# Author: Slaven Rezic
#
...
# XXX Needs a lot of work ... and seems to be slow compared to PDF::Create
# XXX so -> it should not go into the bbbike distribution

package BBBikeDraw::PDF2;
use strict;
...
BEGIN { @colors =
	    ...
	    $darkblue $lightblue $rose $black $darkgrey $lightgreen $lightyellow);
}
use vars @colors;
...
# 	 ),
    ...
    $pdf->info(
	       'Author' => 'Slaven Rezic',
	       'Title' => 'BBBike Route',
	       'Creator' => __PACKAGE__ . " version $BBBikeDraw::PDF2::VERSION",
	       'CreationDate' => [ localtime ],
	       'Keywords' => 'Routenplaner Fahrrad',
	      );
#     $pdf->mediabox("a4");
#     my $page_bbox = $pdf->{MediaBox};
# require Data::Dumper; print STDERR "Line " . __LINE__ . ", File: " . __FILE__ . "\n" . Data::Dumper->new([$page_bbox],[])->Indent(1)->Useqq(1)->Dump; # XXX
...
#XXX	if (($page_bbox->[2]-$page_bbox->[0])/($page_bbox->[3]-$page_bbox->[1]) <
...
#XXX jeht nicht:	$rotate = 90; # oder -90
    ...
    my $page = $pdf->page;#('MediaBox' => $page_bbox,
			   #   (defined $rotate ? ('Rotate' => $rotate) : ()),
			    # );
    $page->mediabox("a4");
    my $page_bbox = [0,0,842,595]; # XXX! $page->{MediaBox}; # XXX
    my $outl = $pdf->outlines;
    my $soutl = $outl->outline;
    $soutl->title("Karte");
    $soutl->dest($page);
    ...
    # grey background
    $page->gfx->rect(@$page_bbox);
    $page->gfx->fillcolor(@$grey_bg);
    $page->gfx->fill;
...
sub allocate_fonts {
    ...
    $sansserif = $self->{PDF}->corefont('Helvetica');
}
...
#	push @outline_netz, $wasser;
	...
	foreach my $strecke (@outline_netz) {
		...
		$im->gfx->linewidth(($width{$cat}||1)*1+2);
		$im->gfx->strokecolor(@{ $outline_color{$cat} || [0,0,0] });
...
#			 ($flaechen_pass == 2 && $cat ne 'F:Pabove'));
		$im->gfx->linewidth(1);
		$im->gfx->strokecolor(@{ $color{$cat} || [0,0,0] });
		$im->gfx->fillcolor  (@{ $color{$cat} || [0,0,0] });
		for my $xy (@{$ss}[1 .. $#$ss]) {
		    $im->gfx->line(@$xy);
		}
		$im->gfx->fill;
		...
		$cat =~ s{::.*}{};
		next if $restrict && !$restrict->{$cat};
		$im->gfx->linewidth(($width{$cat} || 1) * 1);
		$im->gfx->strokecolor(@{ $color{$cat} || [0,0,0] });
		for my $xy (@{$ss}[1 .. $#$ss]) {
		    $im->gfx->line(@$xy);
		}
...
#    		    }
		    ...
		    $im->gfx->fillcolor(@$darkblue);
  		    $im->gfx->rect($x1-2, $y1-2, 5, 5);
		    $im->gfx->fill;
		} elsif ($type eq 's') {
		    # XXX ausgefüllten Kreis zeichnen
		    my($x, $y) = &$transpose(@{$s->coord_as_list(0)});
		    $im->gfx->fillcolor(@$darkgreen);
		    $im->gfx->circle($x, $y, 4);
		    $im->gfx->fill;
		}
#  		} else {
...
sub draw_scale {
    ...
    $im->gfx->strokecolor(@$color);
    $im->gfx->linewidth(1);

    $im->gfx->fillcolor(@$white);
    $im->gfx->rect($self->{Width}-($x1-$x0)-$x_margin,
		   $y_margin,
		   ($x1-$x0)/2,
		   $bar_width);
    $im->gfx->fill;

    $im->gfx->fillcolor(@$color);
    $im->gfx->rect($self->{Width}-($x1-$x0)/2-$x_margin,
		   $y_margin,
		   ($x1-$x0)/2,
		   $bar_width);
    $im->gfx->fill;

    $im->gfx->poly($self->{Width}-($x1-$x0)-$x_margin,
	      $y_margin,
	      $self->{Width}-$x_margin,
	      $y_margin);
    ...
    $im->gfx->poly($self->{Width}-($x1-$x0)/2-$x_margin,
	      $y_margin,
    ...
    $im->gfx->poly($self->{Width}-$x_margin,
	      $y_margin-2,
	      $self->{Width}-$x_margin,
	      $y_margin+$bar_width+2);
...
#      }
    ...
    $im->gfx->strokecolor(@$red);
    $im->gfx->linewidth(4);

    # Route
    if (@c1) {
	$im->gfx->move(map { sprintf "%.2f", $_ } $transpose->(@{ $c1[0] }));
	for(my $i = 1; $i <= $#c1; $i++) {
	    $im->gfx->line(map { sprintf "%.2f", $_ } $transpose->(@{ $c1[$i] }));
	}
	$im->gfx->stroke;
    }
...
$self->{UseFlags} = 0; # XXX don't use this because of missing transparency information in .jpg
	...
	foreach my $e (@strnames) {
	    ...
	    $im->gfx->fillcolor(@$white);
	    $im->gfx->rect($x-2, $y-2, $s_width+4, $size+2);
	    $im->gfx->fill;
	    $im->gfx->rect($x-2, $y-2, $s_width+4, $size+2);
	    $im->gfx->stroke;
	    $im->gfx->fillcolor(@$black);
	    $im->string($sansserif, $size, $x, $y, $name);
	}
    }

    if (0 && $self->{TitleDraw}) {# XXX
	...
	$im->gfx->fillcolor(@$white);
	$im->gfx->linewidth(1);
	$im->gfx->rect(15, 795, $s_width+5+5, $size+5);
	$im->gfx->fill;
	$im->gfx->rect(15, 795, $s_width+5+5, $size+5);
	$im->gfx->stroke;
...
sub draw_wind {
    ...
    my($rad) = 10;
    my $col = $darkblue;
    $im->gfx->strokecolor(@$col);
    $im->gfx->fillcolor(@$col);
    $im->gfx->circle($self->{Width}-20, 20, $rad);
    $im->gfx->fill;
    if ($staerke > 0) {
	...
	if (exists $senkrecht{"$xdir,$ydir"}) {
	    ...
	    $im->gfx->poly($self->{Width}-20, 20, $self->{Width}-20+$xadd, 20+$yadd);
	    my $this_tic = 15;
	    my $i = $staerke;
	    my $last_is_half = 0;
	    if ($i%2 == 1) {
		...
		$xadd2 = -$xadd2;
		$im->gfx->poly($self->{Width}-20+$xadd, 20+$yadd,
...
sub add_route_descr {
    my $self = shift;
    warn "Route::PDF does not work together with BBBikeDraw::PDF2..."; return;
    my(%args) = @_;
...
sub flush {
	...
	$self->{PDF}->saveas($self->{Filename});
    } else {
	my $fh = $args{Fh} || $self->{Fh};
	binmode $fh;
	print $fh $self->{PDF}->stringify;
    }
    $self->{PDF}->end;
}
