...
by Perl/Encode may be used). If not set, then iso-8859-1 is assumed.
...
The next block or line is unsure or unclear information. The I<why>
text specifies an additional comment. This note should not appear in
the processed data. C<XXX> is an alias for C<unsure:>.
...
without frequent traffic, B<F> for pedestrian-only traffic lights and
B<Zbr> for drawbridges, otherwise not used.
