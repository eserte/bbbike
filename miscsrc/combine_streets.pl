...
#
# $Id: combine_streets.pl,v 1.12 2005/04/26 23:12:04 eserte Exp $
# Author: Slaven Rezic
#
...
sub combine_same_streets {
	...
	while(my($p2, $v2) = each %$v) {
	    my @rec = map { $str->get($_) } @$v2;
	    print join(", ", map { $_->[Strassen::NAME] } @rec);
	    print "\t$rec[0]->[Strassen::CAT] ";
	    print join(" ", $p1, $p2);
	    print "\n";
	}
    }
