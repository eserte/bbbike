...
#
# $Id: CombineTrainStreetNets.pm,v 1.2 2003/08/24 23:26:13 eserte Exp $
# Author: Slaven Rezic
#
...
#   in bbbike/ptksh: $net=CombineTrainStreetNets::build_search_net() aufrufen
...
sub build_custom_net {
    ...
    my $bahn_cat_net = StrassenNetz->new($bahn_str);
	...
	my $ret = $s_str->nearest_point($_->[Strassen::COORDS][0],
					FullReturn => 1);
	if ($ret && $ret->{Coord} ne $_->[Strassen::COORDS][0]) {
	    add_net2($net, $ret->{Coord}, $_->[Strassen::COORDS][0]);
	}
    } $stations;
