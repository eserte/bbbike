...
sub gps_mode_change {
	...
	if ($new_mode >= 2) {
		...
		$msg .= " Die Genauigkeit beträgt " . int($current_accuracy) . " Meter.";
	    }
	}
		...
		if (@current_search_route) {
			...
			if ($to_index >= $current_search_route[$j]->[StrassenNetz::ROUTE_ARRAYINX()][0]+1 &&
				...
				if ($live_dist > 500 && !$reported_point{"LONG; THEN $next_street $direction"}) {
				    ...
				    if ($do_speech) {
					set_volume_for_current_point($nearest_object);
					saytext("Dem Straßenverlauf noch $live_dist Meter folgen, dann");
					saydirection(@saydirection_args);
					$reported_point{"LONG; THEN $next_street $direction"} = 1;
				    } else {
					warn "Would say: Dem Straßenverlauf noch $live_dist Meter folgen, dann " . make_german_direction(@saydirection_args) . "\n"; # XXX debug?
				    }
