...
sub add_button {
	...
	(-menuitems =>
	 [
	  [Radiobutton => M"Ca. halbe Bildschirmgrˆﬂe",
	   -variable => \$geometry,
	   -value => "half",
	  ],
	  [Radiobutton => M"Ca. 1/3 der Bildschirmgrˆﬂe",
	   -variable => \$geometry,
	   -value => "third",
	  ],
	  [Radiobutton => M"Halbe Bildgrˆﬂe",
	   -variable => \$geometry,
	   -value => "image-half",
	  ],
	  [Radiobutton => M"1/3 der Bildgrˆﬂe",
	   -variable => \$geometry,
	   -value => "image-third",
	  ],
	  [Radiobutton => M"Maximale Grˆﬂe",
	   -variable => \$geometry,
	   -value => "max",
	  ],
	 ]
    ...
    BBBikePlugin::place_menu_button
		...
		($prog_is_available{"xdg-open"}
		 ...
		 : ()
	      ...
	      [Cascade => M"Viewer f¸r Originalbild",
	       -menuitems =>
	       [
		($^O eq 'MSWin32' ? () :
		 [Radiobutton => M"Bester externer Viewer",
...
sub show_image_viewer {
	...
	if ($name =~ /^Image:\s*\"([^\"]+)\"/) {
	    ...
	    $abs_file = $file; # XXX suffix? aufr‰umen?
	}
	...
	if ($use_viewer eq '_internal') {
		...
		if (!defined $image_viewer_toplevel) {
		    ...
		    $main::balloon->attach($next_button, -msg => M"N‰chstes Bild") if ($main::balloon);
		    ...
		    $main::balloon->attach($close_button, -msg => M"Viewer schlieﬂen") if ($main::balloon);
...
sub orig_editor {
	...
	main::status_message("gimp ist nicht verf¸gbar", "die"); # XXX Msg
    }
...
sub _fill_exif_viewer {
	...
	or main::status_message(($! ? "$!" : M"Fehler beim Ausf√ºhren von exiftool"), "die");
    while(<$fh>) {
	chomp;
